...
Descrizione: Script v2.3 - Importazione completa e resiliente.
Gestisce tutte le anagrafiche, la didattica e i fallback per i dati legacy.
"""
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('  UNIGEST - IMPORTAZIONE DATI COMPLETA (v2.3)'))
        self.stdout.write(self.style.SUCCESS('='*70 + '\n'))

        # Inizializzazione modelli di base
        if not self.dry_run:
            for q in [0, 1, 2, 3]:
        ...
        for section_name, task_func in tasks:
            ...
            try:
                ...
                self.log(f"Errore fatale in {section_name}: {e}", 'error')
    ...
    def import_iscritti(self):
        ...
        cursor.execute("""
            SELECT Matr, `M/F`, Sig, Nominativo, Moglie, Indirizzo, Paese,
                   ...
                   Email, Whatsapp, Posta, TitoloStudio, ProfAtt, ProfPass
            FROM `TAnagrafe`
        """)
        ...
        for row in rows:
            try:
                if not self.dry_run:
                    ...
                    # Mapping Titolo Studio e Professioni
                    ts = TitoloStudio.objects.filter(descrizione=row[16]).first() if row[16] else None
                    pa = ProfessioneAttuale.objects.filter(descrizione=row[17]).first() if row[17] else None
                    pp = ProfessionePassata.objects.filter(descrizione=row[18]).first() if row[18] else None
                    ...
                    Iscritto.objects.get_or_create(matricola=row[0], defaults={
                        ...
                        'codice_fiscale': cf,
                        'email': row[13] or '',
                        'ha_whatsapp': bool(row[14]),
                        'riceve_posta': bool(row[15]),
                        'titolo_studio': ts,
                        'professione_attuale': pa,
                        'professione_passata': pp,
        ...
        for i, row in enumerate(cursor.fetchall()):
            ...
            try:
                if not self.dry_run:
                    ...
                    a_start = int(parti[0]) if parti[0].isdigit() else 2000
    ...
    def import_edizioni(self):
        ...
        cursor.execute("""
            SELECT ID, Anno, Codice, Descrizione, Quadrimestre, Insegnante,
                   Giorni, Dalle, Alle FROM `TCorsiAnnualiDocenti`
        """)
        for row in cursor.fetchall():
            try:
                if not self.dry_run:
                ...
                if not self.dry_run:
                    ...
                    if anno and isc:
                        IscrizioneAnnoAccademico.objects.get_or_create(
                            anno_accademico=anno, iscritto=isc,
        ...
        for row in cursor.fetchall():
            try:
                if not self.dry_run:
                    ...
                    if anno and isc and ediz:
                        IscrizioneCorso.objects.get_or_create(
                            anno_accademico=anno, iscritto=isc, edizione_corso=ediz,
        ...
        for row in cursor.fetchall():
            try:
                if not self.dry_run:
                    ...
                    if ediz and row[1]:
                        doc_id = row[3] if Docente.objects.filter(id=row[3]).exists() else ediz.docente_id
                        Lezione.objects.get_or_create(
                            edizione_corso=ediz, data_lezione=row[1],
                            defaults={'descrizione': row[2] or '', 'docente_id': doc_id, 'numero_presenti': row[4] or 0, 'ore_lezione': float(row[5]) if row[5] else 2.0}
                        )
    ...
    def print_summary(self):
        self.stdout.write('\n' + '='*70)
        self.stdout.write(self.style.SUCCESS(f"  RIEPILOGO IMPORTAZIONE FINALE (v2.3{' - DRY RUN' if self.dry_run else ''})"))
